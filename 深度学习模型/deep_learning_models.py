...
Deep Learning Models: Bidirectional LSTM & GRU for Hong Kong Airport Passenger Traffic
...
print("Models: Bidirectional LSTM, GRU, XGBoost")
print("="*70)
...
class BiLSTMModel(nn.Module):
    ...
    Architecture:
    - 1 Bidirectional LSTM layer with 32 hidden units per direction
    - Processes sequence in both forward and backward directions
    - Concatenates outputs: 32*2 = 64 features
    ...
    - Better at learning temporal dependencies
    """
...
# Training Function
# ============================================================
def train_model(model, model_name, num_runs=3):
    """Train a PyTorch model with early stopping, select best from multiple runs"""
    best_overall = None
    best_mape = float('inf')
    ...
    return model, best_overall['train_losses'], best_overall['val_losses'], best_overall['best_val_loss'], best_overall['stopped_epoch']
...
# ============================================================
# PART 2: Bidirectional LSTM MODEL
# ============================================================
print("\n" + "="*70)
print("PART 2: Bidirectional LSTM MODEL")
print("="*70)
...
[2.1] Model Architecture
      ...
      │    ↓                                                    │
      │  Bidirectional LSTM: {LSTM_HIDDEN} hidden units (×2 directions) │
      │    ↓                                                    │
      │  Concatenate: {LSTM_HIDDEN}×2 = {LSTM_HIDDEN*2} features               │
      │    ↓                                                    │
      │  Fully Connected: {LSTM_HIDDEN*2} → 1                              │
      │    ↓                                                    │
      │  Output: (batch, 1)                                     │
      └─────────────────────────────────────────────────────────┘
""")

lstm_model = BiLSTMModel(input_size, hidden_size=LSTM_HIDDEN, num_layers=LSTM_LAYERS).to(device)
...
print(f"      Multiple runs: 3 (select best)")

print(f"\n[2.4] Training Process")
print(f"      Training Bidirectional LSTM (3 runs, selecting best)...")
lstm_model, lstm_train_losses, lstm_val_losses, lstm_best_val, lstm_stopped = train_model(lstm_model, "BiLSTM", num_runs=3)
print(f"        Stopped at epoch: {lstm_stopped}")
print(f"        Best validation loss: {lstm_best_val:.6f}")

# Save LSTM model
torch.save(lstm_model.state_dict(), f'{DATA_DIR}/lstm_model.pth')

# LSTM Predictions
lstm_predictions = predict_model(lstm_model)
lstm_mae, lstm_rmse, lstm_mape = evaluate_predictions(actual_values, lstm_predictions)

print(f"\n[2.5] Bidirectional LSTM Evaluation Results")
print(f"      ┌────────────────────────────────────────┐")
...
print(f"\n[2.6] Bidirectional LSTM Detailed Predictions")
...
[3.1] Model Architecture
      ...
      │  Input: (batch, {SEQ_LENGTH}, {input_size})                              │
      │    ↓                                                    │
      │  GRU Layer 1: {GRU_HIDDEN} hidden units                         │
      │    ↓                                                    │
      │  Dropout: {GRU_DROPOUT}                                          │
      │    ↓                                                    │
      │  GRU Layer 2: {GRU_HIDDEN} hidden units                         │
      │    ↓                                                    │
      │  Fully Connected: {GRU_HIDDEN} → 1                              │
      │    ↓                                                    │
      │  Output: (batch, 1)                                     │
      └─────────────────────────────────────────────────────────┘

[3.2] GRU vs Bidirectional LSTM Comparison
      ┌────────────────────────────────────────────────────────┐
      │  Feature          │  BiLSTM   │    GRU    │           │
      ├────────────────────────────────────────────────────────┤
      │  Gates            │     3     │     2     │ GRU简化   │
      │  Bidirectional    │    Yes    │    No     │ BiLSTM有  │
      │  Cell State       │    Yes    │    No     │ GRU无     │
      │  Architecture     │  1-layer  │  2-layer  │           │
      └────────────────────────────────────────────────────────┘
""")

gru_model = GRUModel(input_size, hidden_size=GRU_HIDDEN, num_layers=GRU_LAYERS, dropout=GRU_DROPOUT).to(device)
gru_params = sum(p.numel() for p in gru_model.parameters())

print(f"[3.3] Model Parameters")
print(f"      Total parameters: {gru_params:,}")
print(f"      Parameter comparison vs BiLSTM: {(gru_params - lstm_params) / lstm_params * 100:+.1f}%")

print(f"\n[3.4] Training Configuration")
print(f"      (Same as BiLSTM)")

print(f"\n[3.5] Training Process")
print(f"      Training GRU (3 runs, selecting best)...")
gru_model, gru_train_losses, gru_val_losses, gru_best_val, gru_stopped = train_model(gru_model, "GRU", num_runs=3)
print(f"        Stopped at epoch: {gru_stopped}")
print(f"        Best validation loss: {gru_best_val:.6f}")

# Save GRU model
torch.save(gru_model.state_dict(), f'{DATA_DIR}/gru_model.pth')

# GRU Predictions
gru_predictions = predict_model(gru_model)
...
[5.1] Performance Comparison
      ...
      ├─────────────────────────────────────────────────────────────┤
      │  BiLSTM   │   {lstm_params:>8,}  │  {lstm_mae:>7,.0f}  │  {lstm_rmse:>6,.0f}  │  {lstm_mape:>5.2f}%  │
...
results = [
    ('BiLSTM', lstm_mape, lstm_predictions),
...
dl_better = lstm_mape < xgb_mape or gru_mape < xgb_mape
print(f"      • Deep Learning vs ML: {'DL outperforms XGBoost' if dl_better else 'Comparable performance'}")
...
ax1.set_title('Bidirectional LSTM: Training and Validation Loss', fontsize=14)
...
ax3.plot(test_df['Date'], lstm_predictions, 'r--', label='BiLSTM Prediction', 
...
ax3.set_title(f'Bidirectional LSTM Forecast (MAPE: {lstm_mape:.2f}%)', fontsize=14)
...
ax5.plot(x_pos, lstm_predictions, 'rs--', label=f'BiLSTM (MAPE:{lstm_mape:.1f}%)', linewidth=1.5, markersize=7)
...
bars2 = ax6.bar(x_pos - 0.5*width, lstm_predictions, width, label='BiLSTM', color='salmon', alpha=0.8)
...
ax7a.set_title(f'BiLSTM Prediction Error (MAPE: {lstm_mape:.2f}%)', fontsize=12)
...
predictions_df['BiLSTM'] = lstm_predictions
...
print("="*70)
print(f"""
Models Trained: 3 (Bidirectional LSTM, GRU, XGBoost)

Best Model: {best_model} (MAPE: {best_mape:.2f}%)

Bidirectional LSTM Configuration:
  - Architecture: 1-layer Bidirectional LSTM, {LSTM_HIDDEN} hidden units
  - Parameters: {lstm_params:,}
  - Performance: MAE={lstm_mae:,.0f}, MAPE={lstm_mape:.2f}%

GRU Configuration:
  - Architecture: 2-layer GRU, {GRU_HIDDEN} hidden units  
